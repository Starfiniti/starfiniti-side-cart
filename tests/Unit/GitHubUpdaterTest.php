...
final class GitHubUpdaterTest extends TestCase {
	...
	public function test_returns_and_caches_valid_update(): void {
		...
		self::assertSame( '1.4.1', $first['version'] );
	...
	public function test_returns_plugin_information(): void {
		...
		self::assertSame( '1.4.1', $result->version );
	...
	public function test_accepts_checksum_asset_fallback(): void {
		...
		$this->configure_release( $release );
		$GLOBALS['sfcart_test_remote'][ $checksum_url ] = $this->http_response( $checksum . '  starfiniti-cart-1.4.1.zip' );
	...
	private function release( string $checksum = '' ): array {
		...
		return array(
			...
			'draft'      => false,
			'prerelease' => false,
			'html_url'   => 'https://github.com/Starfiniti/starfiniti-side-cart/releases/tag/v1.4.1',
			'body'       => 'Security fixes and update support.',
			'assets'     => array(
				array(
					'name'                 => 'starfiniti-cart-1.4.1.zip',
					'browser_download_url' => $this->package_url(),
					'size'                 => 2048,
					'digest'               => $digest,
				),
				array(
					'name'                 => 'starfiniti-cart-1.4.1.zip.sha256',
	...
	private function package_url(): string {
		return 'https://github.com/Starfiniti/starfiniti-side-cart/releases/download/v1.4.1/starfiniti-cart-1.4.1.zip';
	}
