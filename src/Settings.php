	...
	private static function reward_milestones( mixed $value ): array {
		...
		foreach ( array_slice( $value, 0, 10 ) as $index => $milestone ) {
			...
			$milestones[] = array(
				...
				'pending_message'   => self::text( $milestone['pending_message'] ?? 'Spend {{remaining_amount}} more to unlock {{reward}}.', 250 ),
				'achieved_message'  => self::text( $milestone['achieved_message'] ?? '{{reward}} unlocked.', 250 ),
